...
class SingleExchangeStrategy(Strategy):
    ...
    def limit_order(self):
        ...
        portfolio_USD = 2000.00      #replace with model
        portfolio_EUR = 1600.00
        ...
        (buy_trade,sell_trade) = self.produce_trade_pair_forward(currency_pair_state_USD, currency_pair_state_EUR)
        
        gain_total = self.gain(buy_trade,sell_trade,currency_pair_state_USD,currency_pair_state_EUR)
        ...
        if(gain_total <= 0):
            ...
            new_trade_volume = portfolio_manager(portfolio_EUR,sell_trade.get_volume(),sell_trade.get_price(),gain_total)
            
            buy_trade.set_volume(new_trade_volume)
            sell_trade.set_volume(new_trade_volume)
            return [buy_trade,sell_trade]
            
        new_trade_volume = portfolio_manager(portfolio_USD,sell_trade.get_volume(),sell_trade.get_price(),gain_total)
